...
export class LayerSystem extends ToolWindow {
    ...
    init() {
        ...
        this.CANVAS.backscreen_white_ctx =
            this.CANVAS.backscreen_white.getContext("2d");
        ...
        this.CANVAS.backscreen_trans_ctx =
            this.CANVAS.backscreen_trans.getContext("2d");
        // ワークエリア
    ...
    startEvent() {
        // セレクトボックス：レイヤー合成モード
        document
            .getElementById("axp_layer_select_blendMode")
        ...
        // レンジスライダー：レイヤー不透明度
        document
            .getElementById("axp_layer_range_alpha")
        ...
        const updateLayerName = (nameText) => {
            this.currentLayer.querySelector(
        ...
        // サブウィンドウ
        document
            .getElementById("axp_renamelayer")
            .addEventListener("click", () => {
                cancelRename();
            });
        document
            .querySelector("#axp_renamelayer>div")
        ...
        // キャンセルボタン
        document
            .getElementById("axp_renamelayer_button_close")
            .addEventListener("click", () => {
                cancelRename();
            });
        ...
        const confirmRename = () => {
            ...
            let newName = this.normalizeLayerName(
                document.getElementById("axp_renamelayer_text_newLayerName")
                    .value,
                20,
            );
            if (newName === "") {
        ...
        // 決定ボタン
        document
            .getElementById("axp_renamelayer_button_confirm")
            .addEventListener("click", (e) => {
                // ボタン有効時のみ処理
        ...
        // エンターキーで決定
        document
            .getElementById("axp_renamelayer_text_newLayerName")
        ...
        // 色解除
        document
            .getElementById("axp_renamelayer_button_colorReset")
            .addEventListener("click", () => {
                this.setTag(-1);
        ...
        // カラータグ
        const elementColorTagButton = document.getElementById(
            "axp_renamelayer_div_colorTag",
        );
        const elementColorTagButtonConfig = document.getElementById(
        ...
        document
            .getElementById("axp_renamelayer_button_flipH")
            .addEventListener("click", () => {
                this.flip_h();
                // 選択中レイヤーの左右を反転しました。
        ...
        document
            .getElementById("axp_renamelayer_button_flipV")
            .addEventListener("click", () => {
                this.flip_v();
                // 選択中レイヤーの上下を反転しました。
    ...
    getNewLayerName() {
        ...
        return checkDuplicateLayerName(
            this.axpObj._("@LAYER.NEWLAYER_TEMPLATE"),
        );
    }
    ...
    updateAllColorTag() {
        for (let idx = 0; idx < this.colorTagList.length; idx++) {
            ...
            document.querySelectorAll(".axpc_renamelayer_button_colorTag")[
                idx
            ].textContent = this.colorTagList[idx].name;
            // 設定タブのテキスト表示を更新
            document.querySelectorAll(".axpc_config_text_colorTag")[idx].value =
                this.colorTagList[idx].name;
        }
    }
    ...
    clear(id) {
        ...
        this.layerObj[this.getLayerIndex(id)].image =
            this.CANVAS.tmp_ctx.createImageData(
                this.axpObj.x_size,
                this.axpObj.y_size,
            );
    }
    getLocked(index = null) {
        if (index) {
            ...
            return this.layerObj[
                this.getLayerIndex(this.currentLayer.dataset.id)
            ].locked;
        }
    }
    ...
    getLayerElement(index) {
        const elementLi = document.querySelector(
            `#axp_layer_ul_layerBox>li:nth-of-type(${index + 1})`,
        );
    ...
    // クリッピング状態
    getClupMode() {
        return this.currentLayer.querySelector(".axpc_layer_div_cellCliping")
            .dataset.mode;
    }
    // クリッピングレイヤーの親のレイヤー名
    getClupParentName() {
    ...
    setAlpha(alpha, index = null) {
        if (index) {
            ...
            const elementEye = this.getLayerElement(index).querySelector(
            ...
            // 不透明度に応じて目のアイコンの背景を変化させる
            const elementEye = this.currentLayer.querySelector(
                ".axpc_layer_div_cellButtonEye",
            );
            elementEye.style.background = this.getEyeBackground(alpha);
        }
    ...
    getMode() {
        return this.layerObj[this.getLayerIndex(this.currentLayer.dataset.id)]
            .mode;
    }
    getId() {
        return this.layerObj[this.getLayerIndex(this.currentLayer.dataset.id)]
    ...
    getIndex() {
        ...
        if (this.currentLayer === null) {
            ...
            index =
                this.layerObj[this.getLayerIndex(this.currentLayer.dataset.id)]
                    .index;
        }
        return index;
    }
    ...
    getCanvasImage() {
        if (this.axpObj.assistToolSystem.getIsTransparent()) {
            return this.CANVAS.backscreen_trans_ctx.getImageData(
                0,
                0,
                this.axpObj.x_size,
                this.axpObj.y_size,
            ...
            return this.CANVAS.backscreen_white_ctx.getImageData(
                0,
                0,
                this.axpObj.x_size,
                this.axpObj.y_size,
    ...
    write(imageData) {
        // imagedataの更新
        this.layerObj[this.getLayerIndex(this.currentLayer.dataset.id)].image =
            imageData;
    }
    ...
    setName(name) {
        this.layerObj[this.getLayerIndex(this.currentLayer.dataset.id)].name =
            name;
    }
    setBlendMode(blendMode) {
        this.layerObj[this.getLayerIndex(this.currentLayer.dataset.id)].mode =
            blendMode;
        // レイヤーセルに合成モードを表示
        this.displayBlendMode(this.currentLayer);
    }
    setTag(tagNumber) {
        this.layerObj[this.getLayerIndex(this.currentLayer.dataset.id)].tag =
            tagNumber;
    }
    displayBlendMode(elementLayer) {
        const elementStatus = elementLayer.querySelector(
            ".axpc_layer_span_cellStatus",
        );
        const blendMode =
            this.layerObj[this.getLayerIndex(elementLayer.dataset.id)].mode;
        if (
            ...
            if (
                ...
                const option = document.querySelector(
                    `#axp_layer_select_blendMode>option[value='${blendMode}']`,
                );
                // optionの表示名を取得
        ...
        // クリッピング時用のdataset設定
        const elementCliping = elementLayer.querySelector(
            ".axpc_layer_div_cellCliping",
        );
        elementCliping.dataset.mode = blendMode;
        const elementRightSide = elementLayer.querySelector(
    ...
    updateBlendModeDisplayAll() {
        const elementsLayer = document.querySelectorAll(
    ...
    isWriteProtection(index = null) {
        if (
            this.getLocked(index) ||
            !this.getChecked(index) ||
            this.getAlpha(index) == 0
        ) {
            return true;
        } else {
            return false;
    ...
    newLayer() {
        ...
        let layerData = {
            ...
            tag: -1,
            // 空のイメージを生成
            image: this.CANVAS.tmp_ctx.createImageData(
                this.axpObj.x_size,
                this.axpObj.y_size,
            ),
    ...
    restoreLayer(actionObj) {
        let layerData = {
            ...
            tag:
                actionObj.layerObj.tag === undefined
                    ? -1
                    : actionObj.layerObj.tag,
            // 元のイメージを復元
    ...
    createLayer(layerData) {
        ...
        divElement.insertBefore(newLayer, divElement.children[insert_idx]);
    ...
    createLayerHTML(layerData) {
        ...
        // レイヤー表示状態の切り替え
        newDivEyeIcon.addEventListener(
            "pointerenter",
            (e) => {
                // ポインターが進入した時
                let msgtext = e.target.classList.contains("axpc_icon_eyeON")
                    ? "表示"
                    : "非表示";
                ...
                if (e.pointerType === "mouse") {
                    if (e.buttons === 1) {
                        if (!this.axpObj.isDrawing) {
                            if (
                                e.target.classList.contains("axpc_icon_eyeON")
                            ) {
                                console.log("off");
                                e.target.classList.remove("axpc_icon_eyeON");
                                e.target.classList.add("axpc_icon_eyeOFF");
                                this.layerObj[
                                    this.getLayerIndex(e.target.dataset.id)
                                ].checked = false;
                            } else {
                                console.log("on");
                            ...
                            msgtext = e.target.classList.contains(
                                "axpc_icon_eyeON",
                            )
                                ? "表示"
                                : "非表示";
        ...
        newDivEyeIcon.addEventListener(
            ...
            (e) => {
                ...
                var msgtext = e.target.classList.contains("axpc_icon_eyeON")
                    ? "表示"
                    : "非表示";
                // レイヤーの%1を切り替えました。(現在の状態:%2)
                this.axpObj.msg("@INF4006", "表示状態", msgtext);
        ...
        if (layerData.locked) {
            newCheckbox_lock.setAttribute(
                "class",
                "axpc_layer_div_cellButton axpc_icon_lockON",
            );
        } else {
            newCheckbox_lock.setAttribute(
                "class",
                "axpc_layer_div_cellButton axpc_icon_lockOFF",
            );
        }
        newCheckbox_lock.setAttribute("data-id", layerData.id);

        newCheckbox_lock.addEventListener(
            "pointerenter",
            (e) => {
                // ポインターが進入した時
                var msgtext = e.target.classList.contains("axpc_icon_lockON")
                    ? "ロック"
                    : "なし";
        ...
        if (layerData.masked) {
            ...
            newCheckbox_mask.setAttribute(
                "class",
                "axpc_layer_div_cellButton axpc_icon_maskON",
            );
        } else {
            newCheckbox_mask.setAttribute(
                "class",
                "axpc_layer_div_cellButton axpc_icon_maskOFF",
            );
        }
        newCheckbox_mask.setAttribute("data-id", layerData.id);
        newCheckbox_mask.addEventListener(
            "pointerenter",
            (e) => {
                // ポインターが進入した時
                var msgtext = e.target.classList.contains("axpc_icon_maskON")
                    ? "ロック"
                    : "なし";
                // レイヤーの%1を切り替えます。(現在の状態:%2)
        ...
        newCanvas.setAttribute(
            "class",
            "axpc_layer_canvas_cellThumbnail axpc_layer_CANDRAG",
        );
        ...
        newSpan1.setAttribute(
            "class",
            "axpc_layer_span_cellName axpc_layer_CANDRAG",
        );
        newSpan1.textContent = layerData.name;

        // レイヤーの状態
        let newSpan2 = document.createElement("span");
        newSpan2.setAttribute(
            "class",
            "axpc_layer_span_cellStatus axpc_layer_CANDRAG",
        );
        ...
        newDivRight.setAttribute(
            "class",
            "axpc_layer_div_cellRightSide axpc_layer_CANDRAG",
        );
        ...
        newDivCliping.setAttribute(
            "class",
            "axpc_layer_div_cellCliping axpc_layer_CANDRAG",
        );
        ...
        newDivtTagColor.setAttribute(
            "class",
            "axpc_layer_div_cellColorTag axpc_layer_CANDRAG",
        );
    ...
    openRenameLayerWindow() {
        ...
        // 現在のレイヤー名を取得
        const elementLayerLabel = this.currentLayer.querySelector(
            ".axpc_layer_span_cellName",
        );
        ...
        const onPointerUp = () => {
            ...
            if (idx !== data.idx_source) {
                ...
                */
                ...
                let moveCtx = this.CANVAS.thumbnail_ctx.splice(
                    data.idx_source,
                    1,
                );
                ...
                // safari
                if (this.axpObj.ENV.multiCanvas) {
                    let movelayerCtx = this.CANVAS.layer_ctx.splice(
                        data.idx_source,
                        1,
                    );
    ...
    setCurrentLayer(targetElement) {
        ...
        const layerBoxElements = document.querySelectorAll(
            "#axp_layer_ul_layerBox>li",
        );
        // 一旦、全レイヤーを非選択に
        ...
        // 合成モードの復元処理
        document.getElementById("axp_layer_select_blendMode").value =
            this.getMode();
    ...
    draw() {
        ...
        this.CANVAS.backscreen_trans_ctx.clearRect(
            0,
            0,
            this.x_size,
            this.y_size,
        ...
        if (!this.axpObj.assistToolSystem.getIsTransparent()) {
            ...
            this.CANVAS.backscreen_trans_ctx.fillRect(
                0,
                0,
                this.x_size,
                this.y_size,
        ...
        for (let idx = this.layerObj.length - 1; idx >= 0; idx--) {
            ...
            this.CANVAS.thumbnail_ctx[idx].clearRect(
                0,
                0,
                clearRect,
                clearRect,
            ...
            if (item.mode === "source-atop") {
                ...
                // 無効なクリッピングであることを表示
                const elementCliping = this.getLayerElement(idx).querySelector(
                    ".axpc_layer_div_cellCliping",
                );
                ...
                if (idx !== 0) {
                    if (this.layerObj[idx - 1].mode === "source-atop") {
                        ...
                        // クリップ領域の初期化
                        this.CANVAS.clip_ctx.globalCompositeOperation =
                            "source-over";
                        this.CANVAS.clip_ctx.globalAlpha = 1;
                        this.CANVAS.clip_ctx.clearRect(
                            0,
                            0,
                            this.axpObj.x_size,
                            this.axpObj.y_size,
                        ...
                        while (skipIdx >= 0) {
                            if (this.layerObj[skipIdx].mode === "source-atop") {
                                ...
                                // 有効なクリッピングであることを表示
                                const elementCliping = this.getLayerElement(
                                    skipIdx,
                                ).querySelector(".axpc_layer_div_cellCliping");
                                elementCliping.dataset.mode =
                                    this.layerObj[skipIdx].mode;

                                // 子レイヤーが「表示」の場合、そのレイヤーを親と合成する
                                if (this.layerObj[skipIdx].checked) {
                                    // safari
                                    if (this.axpObj.ENV.multiCanvas) {
                                        this.CANVAS.layer_ctx[
                                            skipIdx
                                        ].putImageData(
                                            this.layerObj[skipIdx].image,
                                            0,
                                            0,
                                        );
                                        this.CANVAS.clip_ctx.globalAlpha =
                                            this.layerObj[skipIdx].alpha / 100;
                                        this.CANVAS.clip_ctx.drawImage(
                                            this.CANVAS.layer_ctx[skipIdx]
                                                .canvas,
                                            0,
                                            0,
                                        );
                                    } else {
                                        tmp_ctx.putImageData(
                                            this.layerObj[skipIdx].image,
                                            0,
                                            0,
                                        );
                        ...
                        // 描画領域の初期化
                        this.CANVAS.merge_ctx.clearRect(
                            0,
                            0,
                            this.axpObj.x_size,
                            this.axpObj.y_size,
                        ...
                        // 子と合成
                        this.CANVAS.merge_ctx.globalCompositeOperation =
                            "source-atop";
                        this.CANVAS.merge_ctx.globalAlpha = 1;
                        this.CANVAS.merge_ctx.drawImage(
                            this.CANVAS.clip_ctx.canvas,
                            0,
                            0,
                        );
                ...
                if (item.checked) {
                    ...
                    this.CANVAS.backscreen_trans_ctx.globalCompositeOperation =
                        item.mode;
                    this.CANVAS.backscreen_trans_ctx.globalAlpha =
                        item.alpha / 100;
                    this.CANVAS.backscreen_trans_ctx.drawImage(
                        outputCanvas,
                        0,
                        0,
                    );
        ...
        this.CANVAS.backscreen_white_ctx.clearRect(
            0,
            0,
            this.x_size,
            this.y_size,
        ...
        this.CANVAS.backscreen_white_ctx.fillRect(
            0,
            0,
            this.x_size,
            this.y_size,
        );
        this.CANVAS.backscreen_white_ctx.drawImage(
    ...
    buttonIntegrateLayer() {
        ...
        if (this.isWriteProtection()) {
            ...
            this.axpObj.msg(
                "@CAU4006",
                source_name,
                this.getReasonTextForWriteProtection(),
            );
            return;
        }
        // 書き込み禁止チェック（統合先）
        if (this.isWriteProtection(idx_dest)) {
            ...
            this.axpObj.msg(
                "@CAU4006",
                dest_name,
                this.getReasonTextForWriteProtection(idx_dest),
            );
            return;
        }
        ...
        if (this.layerObj[idx_dest].masked) {
            // マスクデータの作成
            this.CANVAS.clip_ctx.putImageData(
                this.layerObj[idx_dest].image,
                0,
                0,
            );
            const imageData = this.CANVAS.clip_ctx.getImageData(
                0,
                0,
                this.axpObj.x_size,
                this.axpObj.y_size,
            ...
            this.CANVAS.merge_ctx.globalAlpha = Number(
                this.layerObj[idx_source].alpha / 100,
            );
            ...
            draw_ctx.globalAlpha = 1;
            if (
                this.layerObj[idx_source].mode === "source-atop" &&
                this.layerObj[idx_dest].mode === "source-atop"
            ) {
                draw_ctx.globalCompositeOperation = "source-over";
            } else {
                draw_ctx.globalCompositeOperation =
                    this.layerObj[idx_source].mode;
            }
            draw_ctx.drawImage(this.CANVAS.merge_ctx.canvas, 0, 0);
        }

        this.layerObj[idx_dest].image = draw_ctx.getImageData(
            0,
            0,
            draw.width,
            draw.height,
    ...
    buttonCopyLayer() {
        ...
        var id_dest = this.copyLayer(
            this.layerObj[this.getLayerIndex(id_source)],
        );
    ...
    buttonClearLayer() {
        ...
        this.axpObj.undoSystem.setUndo({
            type: "layer-clear",
            layerObj:
                this.axpObj.layerSystem.layerObj[
                    this.axpObj.layerSystem.getLayerIndex(this.getId())
                ],
        });
...
const util = {
    ...
    swap(target) {
        ...
        const siblings = parent.querySelectorAll(
            `:scope > *:not(.axpc_onGRAB):not(.${data.cloneName})`,
        );
        ...
        for (let thatIdx = 0, len = siblings.length; thatIdx < len; thatIdx++) {
            ...
            const hitX =
                thatRectX <= targetRectX + targetW &&
                thatRectX + thatW >= targetRectX;
            const hitY =
                targetRectY <= thatRectYHalf &&
                targetRectY + targetH >= thatRectYHalf;
