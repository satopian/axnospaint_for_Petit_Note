...
export class AXPObj {
  ...
  CONST = {
    ...
    MIN_SYSTEM_WIDTH: 8,
    MIN_SYSTEM_HEIGHT: 8,
    MAX_SYSTEM_WIDTH: 2000,
    MAX_SYSTEM_HEIGHT: 2000,
    CANVAS_X_MAX: 800,
    CANVAS_Y_MAX: 800,
    CANVAS_X_MIN: 8,
    CANVAS_Y_MIN: 8,
    ...
    SCALE_MIN: 25,
    SCALE_VALUE: [
      25, 33, 50, 66, 100, 150, 200, 250, 300, 400, 600, 800, 1200, 1600,
    ],
    SCALE_TABLE_MAX: 50,
  ...
  constructor(additionalDictionaryJSON) {
    ...
    this.toolWindow.push(
      (this.colorPaletteSystem = new ColorPaletteSystem(this)),
    );
    ...
    // カスタムボタンは別枠
    this.toolWindow.push(
      (this.customButtonSystem = new CustomButtonSystem(this)),
    );
  ...
  init() {
    ...
    this.ELEMENT.view = document.getElementById(
  ...
  startEvent() {
    ...
    this.debugLog = new DebugLog(
      ...
      document.getElementById('axp_config_checkbox_useDebugMode').checked,
    );
    ...
    // 長押し時のルーペ表示を抑止
    this.ELEMENT.view.addEventListener(
      'touchstart',
      (e) => {
        e.preventDefault();
      },
    ...
    this.ELEMENT.base.addEventListener('pointerleave', () => {
      this.ELEMENT.cursor.style.visibility = 'hidden';
    });
    ...
    this.ELEMENT.base.addEventListener('pointerdown', (e) => {
      ...
      if (this.longPressTimerID) {
        // タッチタイマー終了
        this.debugLog.log(
          `[TIMER] longPressTimerID:${this.longPressTimerID} -> CANCEL(DOWN)`,
        );
        // 長押しキャンセル
      ...
      if (e.pointerType === 'touch') {
        ...
        if (this.evCache.length === 0) {
          const time = Number(
            document.getElementById('axp_config_form_touchDurationValue').volume
              .value,
          );
          // タイマーセット
      ...
      if (e.isPrimary) {
        ...
        if (e.buttons === 2 || e.buttons === 4) {
          ...
          const task =
            e.buttons === 2
              ? this.config('axp_config_form_mouseRightButton')
              : this.config('axp_config_form_mouseWheelButton');
          switch (task) {
            case 'undo':
              this.TASK['func_undo']();
              break;
        ...
        if (
          this.config('axp_config_form_touchDrawType') === 'none' &&
          e.pointerType === 'touch'
        ) {
          // タッチ無効時は描画しない
        } else {
          if (
            this.config('axp_config_form_touchDrawType') === 'hand' &&
            e.pointerType === 'touch'
          ) {
            mode = 'axp_penmode_hand';
          }
        ...
        if (this.config('axp_config_form_useLongtap') === 'on') {
          const time = Number(
            document.getElementById('axp_config_form_longtapDurationValue')
              .volume.value,
          );
          this.longPressTimerID = setTimeout(() => {
            ...
            this.debugStatus();
            this.debugLog.log(
              `[TIMER] longPressTimerID:${this.longPressTimerID} -> TIMEOUT(${time}ミリ秒長押し)`,
            );
          ...
          this.debugLog.log(
            `[TIMER] longPressTimerID:${this.longPressTimerID} -> SET`,
          );
        }
      }
      // [DEBUG]
      this.debugStatus();
      const text = `[DOWN_] ID:${e.pointerId}(${e.pointerType}) P:${e.isPrimary ? 'O' : '-'} (${Math.trunc(e.clientX)},${Math.trunc(e.clientY)})`;
    ...
    this.ELEMENT.base.addEventListener('pointermove', (e) => {
      ...
      if (e.isPrimary) {
        ...
        if (this.longPressTimerID) {
          ...
          if (
            distance <
            Number(
              document.getElementById('axp_config_form_longtapStabilizerValue')
                .volume.value,
            )
          )
            return;
          this.debugLog.log(
            `[TIMER] longPressTimerID:${this.longPressTimerID} -> CANCEL(MOVE)`,
          );
          // 長押しキャンセル
          clearTimeout(this.longPressTimerID);
          this.longPressTimerID = null;
            ...
            if (!isInvalid && this.layerSystem.getMasked()) {
              ...
              if (isCliping) {
                ...
                this.msg(
                  '@INF0008',
                  name,
                  this.layerSystem.getClupParentName(),
                );
              }
            }
          }
        ...
        // 座標表示
        document.getElementById('axp_canvas_div_pointerPosition').textContent =
          textDisplayPositon;
        // 機能呼び出し
      ...
      if (e.pointerType === 'touch') {
        ...
        if (this.touchTimerID && this.isDrawn) {
          // タッチタイマー終了
          this.debugLog.log(
            `[TIMER] touchTimerID:${this.touchTimerID} -> CANCEL(描画済み)`,
          );
        ...
        if (this.evCache.length === 2) {
          if (this.isDrawCancel) {
            ...
            // 座標の変化がある入力があったか？
            const isMoveing =
              Math.abs(diffDistance) + Math.abs(diffX) + Math.abs(diffY) >
              Number(
                document.getElementById('axp_config_form_touchThresholdValue')
                  .volume.value,
              );
            ...
            if (this.touchTimerID) {
              ...
              if (isMoveing) {
                this.debugLog.log(
                  `[TIMER] touchTimerID:${this.touchTimerID} -> CANCEL(スワイプ開始)`,
                );
                clearTimeout(this.touchTimerID);
                this.touchTimerID = null;
              }
            ...
            if (this.touchTimerID === null) {
              ...
              if (this.config('axp_config_form_touchZoom') === 'on') {
                ...
                this.scale = adjustInRange(
                  this.baseScale + diffDistance,
                  this.CONST.SCALE_MIN,
                  this.CONST.SCALE_MAX,
                );
              }
              // カメラ位置移動
              if (this.config('axp_config_form_touchHand') === 'on') {
                this.cameraX = Math.round(
                  this.baseCameraX + (diffX * 100) / this.scale,
                );
                this.cameraY = Math.round(
                  this.baseCameraY + (diffY * 100) / this.scale,
                );
              }
              // キャンバス表示更新
              this.refreshCanvas();
    ...
    const removeEvent = (e) => {
      ...
      if (this.longPressTimerID) {
        this.debugLog.log(
          `[TIMER] longPressTimerID:${this.longPressTimerID} -> CANCEL(UP)`,
        );
        clearTimeout(this.longPressTimerID);
        this.longPressTimerID = null;
      }
    ...
    this.ELEMENT.base.addEventListener('pointerup', (e) => {
      ...
      if (e.pointerType === 'touch') {
        ...
        if (this.evCache.length === 0) {
          if (this.touchTimerID) {
            this.debugLog.log(
              `[TIMER] touchTimerID:${this.touchTimerID} -> CANCEL(タッチ数=0)`,
            );
            // タッチタイマー終了
            clearTimeout(this.touchTimerID);
            this.touchTimerID = null;
            switch (this.fingerCount) {
              case 2:
                if (this.config('axp_config_form_touchUndo') === 'on') {
                  this.debugLog.log(
                    `[EXEC_] アンドゥ実行(最大タッチ数=${this.fingerCount})`,
                  );
                  this.isDrawCancel = true;
                  this.TASK['func_undo']();
                }
              ...
              case 3:
                if (this.config('axp_config_form_touchRedo') === 'on') {
                  this.debugLog.log(
                    `[EXEC_] リドゥ実行(最大タッチ数=${this.fingerCount})`,
                  );
                  this.isDrawCancel = true;
                  this.TASK['func_redo']();
                }
      ...
      // [DEBUG]
      this.debugStatus();
      const text = `[UP___] ID:${e.pointerId}(${e.pointerType}) P:${e.isPrimary ? 'O' : '-'} (${Math.trunc(e.clientX)},${Math.trunc(e.clientY)})`;
      this.debugLog.log(text);
    });
    // ポインタが無効になった時の処理
      ...
      item.addEventListener('pointerenter', (e) => {
        ...
        switch (e.currentTarget.dataset.function) {
          ...
          case 'func_switch_hand':
            ...
            additionalInfo = Message.getMessage(
              e.currentTarget.dataset.addmsg,
            ).slice(2);
          ...
          case 'func_switch_transparent':
            key = this.configSystem.getShortcutFunction(
    ...
    // メインのタブ制御
    this.selectTab('0');
    const elementsTab = document.querySelectorAll(
  ...
  refreshCanvas() {
    ...
    // キャンバスの表示座標更新
    this.CANVAS.main.style.left =
      Math.round(
        centerX - ((this.x_size / 2 + this.cameraX) * this.scale) / 100,
      ) + 'px';
    this.CANVAS.main.style.top =
      Math.round(
        centerY - ((this.y_size / 2 + this.cameraY) * this.scale) / 100,
      ) + 'px';
    // 拡大率に応じたキャンバズサイズ更新
    ...
    // 拡大率数値表示
    document.getElementById('axp_tool_button_loupeReset').textContent =
  ...
  updateGrid() {
    const gridRect = document
      .getElementById('axp_canvas_div_grid')
      .getBoundingClientRect();
    const svg = document.getElementById('axp_canvas_svg_grid');

    // 作成済み補助線の消去
    const elementsRect = svg.getElementsByTagNameNS(
    ...
    const updatePatternVH = (element, h, v, color) => {
      ...
      path.setAttribute(
        'd',
        `M ${div_width} 0 L ${div_width} ${div_height} L 0 ${div_height}`,
      );
      path.setAttribute('stroke', color);
    };
    // 主線
    if (
      document.getElementById('axp_tool_checkbox_gridVH').dataset.checked ===
      'true'
    ) {
      updatePatternVH(
      ...
      svg.appendChild(createRect('axp_canvas_pattern_gridVH'));
    }
    // 副線
    if (
      document.getElementById('axp_tool_checkbox_gridSubDivision').dataset
        .checked === 'true'
    ) {
      updatePatternVH(
        ...
        document.getElementById('axp_tool_color_gridSubDivision').dataset
          .colorcode,
      );
      svg.appendChild(createRect('axp_canvas_pattern_gridSubDivision'));
    }
    ...
    const updatePatternDiagonal = (element, h, v, color) => {
      ...
      path.setAttribute(
        'd',
        `M 0 0 L ${div_width} ${div_height} M ${div_width} 0 L 0 ${div_height}`,
      );
      path.setAttribute('stroke', color);
    };
    // 斜線
    if (
      document.getElementById('axp_tool_checkbox_gridDiagonal').dataset
        .checked === 'true'
    ) {
      updatePatternDiagonal(
        ...
        parseInt(document.getElementById('axp_tool_range_gridH').value),
        parseInt(document.getElementById('axp_tool_range_gridV').value),
        document.getElementById('axp_tool_color_gridDiagonal').dataset
          .colorcode,
      );
      svg.appendChild(createRect('axp_canvas_pattern_gridDiagonal'));
    }
    ...
    const updatePatternPX = (element, scalePx, color) => {
      ...
      path.setAttribute(
        'd',
        `M ${scalePx} 0 L ${scalePx} ${scalePx} L 0 ${scalePx}`,
      );
      path.setAttribute('stroke', color);
    };
    // ピクセル指定１
    if (
      document.getElementById('axp_tool_checkbox_gridPixel1').dataset
        .checked === 'true'
    ) {
      // 補助線の細かさに対して、表示に十分な拡大率である場合表示する
      if (
        (this.scale / 800) *
          document.getElementById('axp_tool_number_gridPixel1').value >=
        1
      ) {
        updatePatternPX(
          ...
          (parseInt(
            ...
            this.scale) /
            100,
          document.getElementById('axp_tool_color_gridPixel1').dataset
            .colorcode,
        );
        svg.appendChild(createRect('axp_canvas_pattern_gridPixel1'));
      }
    }
    // ピクセル指定２
    if (
      document.getElementById('axp_tool_checkbox_gridPixel2').dataset
        .checked === 'true'
    ) {
      // 補助線の細かさに対して、表示に十分な拡大率である場合表示する
      if (
        (this.scale / 800) *
          document.getElementById('axp_tool_number_gridPixel2').value >=
        1
      ) {
        updatePatternPX(
          ...
          (parseInt(
            ...
            this.scale) /
            100,
          document.getElementById('axp_tool_color_gridPixel2').dataset
            .colorcode,
        );
        svg.appendChild(createRect('axp_canvas_pattern_gridPixel2'));
      }
    }
  }
  // 拡大率変更
  ...
  calcScaleCoordinates(e) {
    ...
    let calcX = Math.floor(
      ((e.clientX - clientRect_draw.left) * 100) / this.scale,
    );
    let calcY = Math.floor(
      ((e.clientY - clientRect_draw.top) * 100) / this.scale,
    );
    return {
      x: calcX,
      y: calcY,
  ...
  mouseWheel(e) {
    ...
    // 回転方向反転反転が設定されている場合
    if (
      document.getElementById('axp_config_checkbox_mouseWheelDirection').checked
    ) {
      deltaX = -deltaX;
      deltaY = -deltaY;
    }
    ...
    const sleepTime = Number(
      document.getElementById('axp_config_number_mouseWheelSleepTime').value,
    );
    ...
    switch (this.config('axp_config_form_mouseWheelRotate')) {
      ...
      case 'loupe':
        ...
        if (
          document.getElementById(
            'axp_config_checkbox_mouseWheelPointerTracking',
          ).checked
        ) {
          adjustCamera();
        }
        ...
        // ペンカーソル表示
        if (
          e.target.id === this.CANVAS.main.id ||
          e.target.id === this.ELEMENT.view.id
        ) {
      ...
      case 'scroll': {
        ...
        const move_size = Number(
          document.getElementById('axp_config_number_mouseWheelMoveSize').value,
        );
        if (deltaY < 0) {
          //奥回転
  ...
  drawPostCanvas() {
    ...
    if (isTrans) {
      ...
      this.postSystem.CANVAS.post_ctx.drawImage(
        this.layerSystem.CANVAS.backscreen_trans,
        0,
        0,
      );
      // サムネ
      this.postSystem.CANVAS.thumbnail_ctx.clearRect(
        0,
        0,
      ...
      this.postSystem.CANVAS.thumbnail_ctx.fillRect(
        0,
        0,
        this.postSystem.CANVAS.thumbnail.width,
        this.postSystem.CANVAS.thumbnail.height,
  ...
  selectTab(idx) {
    ...
    const elemHamburger = document.getElementById(
    ...
    if (idx == '2') {
      ...
      if (!this.FUNCTION.post) {
        alert(
          '投稿用プログラムが正しく設定されていないため、投稿することができません。',
        );
        return;
      }
    }
    ...
    // 全タブの選択状態解除
    var targetElements_tab = document.querySelectorAll(
      '#axp_main_div_tab_menu > div',
    );
    ...
    var targetElements_article = document.querySelectorAll(
    ...
    switch (idx) {
      ...
      case '1':
        ...
        this.configSystem.dispPalettebox(
          document.getElementById('axp_config_div_paletteBox'),
          this.colorPaletteSystem.currentPalette,
        );
        break;
      // 投稿
      case '2': {
        ...
        // 基にしてお絵カキコ
        let elemRefId = document.getElementById(
          'axp_post_span_referenceOekakiId',
        );

        console.log(
          'Oekaki ID: %s, Draft Image File: %s',
          String(this.oekaki_id),
          this.draftImageFile,
        );
  ...
  get selectedTab() {
    let result = null;
    const targetElements_tab = document.querySelectorAll(
      '#axp_main_div_tab_menu > div',
    );
  ...
  callTask(id, inkey, repeat = false, code = null) {
    ...
    const inputScaleValue = elem.querySelector(
  ...
  initTask() {
    ...
    this.TASK['func_switch_toggle'] = (inkey) => {
      const isNotPen =
        document.getElementById('axp_pen_button_penBase').dataset.selected !==
        'true';
      // ペン以外が選択されている時はペンに、ペンが選択されているときは消しゴムに切り替え
      if (isNotPen) {
        this.penSystem.switchMainButton(
          document.getElementById('axp_pen_button_penBase'),
          inkey,
        );
      } else {
        this.penSystem.switchMainButton(
          document.getElementById('axp_pen_button_eraserBase'),
          inkey,
        );
      }
      let shortcut = '';
    ...
    this.TASK['func_backgroundimage'] = () => {
      ...
      if (this.isBackgroundimage) {
        this.url_backgroundimage =
          this.layerSystem.CANVAS.backscreen_white.toDataURL('image/png');
        this.ELEMENT.view.style.backgroundImage =
          'url(' + this.url_backgroundimage + ')';
      } else {
    ...
    this.TASK['func_swap_pixelated'] = () => {
      ...
      // コンフィグ保存
      this.configSystem.saveConfig(
        `RADIO_axp_config_form_antialiasing`,
        isTurnON ? 'on' : 'off',
      );
    ...
    const func_scroll = (x, y) => {
      const move_size = Number(
        document.getElementById('axp_config_number_moveSize').value,
      );
      // 方向（設定で反転がチェックされている場合、移動方向を反転させる）
      const move_vector = document.getElementById(
        'axp_config_checkbox_moveDirection',
      ).checked
        ? -1
        : 1;
      this.moveCanvas(x * move_size * move_vector, y * move_size * move_vector);
    };
    // 画面の上スクロール
      ...
      if (this.assistToolSystem.isGrid) {
        ...
        switch (inkey) {
          ...
          case 'up_v':
            ...
            // 連動
            if (
              document.getElementById('axp_tool_checkbox_gridVHLink').checked
            ) {
              div_h = div_v;
            }
            break;
    ...
    this.TASK['func_size_change'] = (inkey, code) => {
      if (!inkey) {
        throw new Error(
          '内部エラー：引数にキーが指定されていません（キーボード専用機能です）',
        );
      }
  ...
  msg(text, ...addText) {
    ...
    replaceText = replaceText.replace(
      '%drawingColorName',
      this.colorMakerSystem.drawingColorName,
    );
    replaceText = replaceText.replace(
      '%addPaletteName',
      this.colorMakerSystem.addPaletteName,
    );
  ...
  drawBackground() {
    ...
    let url_newimage =
      this.layerSystem.CANVAS.backscreen_white.toDataURL('image/png');
    this.ELEMENT.view.style.backgroundImage =
      'url(' + url_newimage + ')' + ',url(' + this.url_backgroundimage + ')';
    ...
    (async () => {
      ...
      if (await this.saveSystem.initDB()) {
        ...
        try {
          ...
          console.log(error);
          alert(
            'エラー:ユーザー設定の読み込みに失敗しました。デフォルト設定で起動します。',
          );
        }
        ...
        if (this.config('axp_config_form_saveLastPalleteColor') === 'off') {
          ...
          try {
            ...
            console.log(error);
            alert(
              'エラー:ユーザーパレットの読み込みに失敗しました。デフォルト設定で起動します。',
            );
          }
        }
      }
      ...
      // アンドゥ使用可能最大数
      this.undo_max = document.getElementById(
        'axp_config_form_undoMaxValue',
      ).result.value;
      // カスタムボタンツールウィンドウ表示切替
      ...
      // キーカスタマイズに拡大率テーブルを反映
      this.configSystem.updateKeyCustomizationScaleTable(
        this.currentScaleTable,
      );
      // キーカスタマイズの折りたたみ
      ...
      if (this.ENV.isFirstLaunch && this.ENV.isMobileWidth) {
        document.getElementById(
          'axp_config_checkbox_singleWindowMode',
        ).checked = true;
        this.configSystem.saveConfig(
          'CHECK_axp_config_checkbox_singleWindowMode',
          true,
        );
        // 単一ウィンドウモード
      ...
      if (isDraftLoaded) {
        ...
        // レイヤー更新
        this.layerSystem.write(
          this.layerSystem.CANVAS.tmp_ctx.getImageData(
            0,
            0,
            this.x_size,
            this.y_size,
  ...
  dispCustomButton() {
    const list = document.querySelectorAll(
      '#axp_config_div_customButtonFunction > select,#axp_config_div_customButtonFunction > input',
    );
    if (this.config('axp_config_form_useCustomButton') === 'on') {
      ...
      document.getElementById(
        'axp_config_div_customButtonFunction',
      ).style.opacity = '1';
    } else {
  ...
  openSubwindow = (id, baseElement) => {
    ...
    if (
      canvasRect.top + canvasRect.height - baseElementRect.bottom >
      windowHeight
    ) {
      // 要素の下側
