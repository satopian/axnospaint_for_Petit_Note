...
export class PenObj {
  ...
  blur(level) {
    ...
    if (blur_value > 0) {
      ...
      if (this.CANVAS.draw_ctx.globalAlpha !== 1) {
        this.CANVAS.draw_ctx.globalAlpha =
          this.CANVAS.draw_ctx.globalAlpha / Math.sqrt(2);
      }
    }
  ...
  drawCursor(e) {
    ...
    if (this.usePenGuide) {
      ...
      if (this.borderStyle === 'normal') {
        // 透明色を考慮するパターン指定
        if (
          document.getElementById('axp_makecolor_div_transparent').dataset
            .selected === 'true'
        ) {
          // 透明色の場合、破線にする
  ...
  write() {
    ...
    this.axpObj.layerSystem.write(
      this.CANVAS.draw_ctx.getImageData(
        0,
        0,
        this.axpObj.x_size,
        this.axpObj.y_size,
