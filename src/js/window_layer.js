...
export class LayerSystem extends ToolWindow {
  ...
  startEvent() {
    ...
    const updateLayerName = (nameText) => {
      this.currentLayer.querySelector('.axpc_layer_span_cellName').textContent =
        nameText;
    };
    // 画面のタグ色更新
    ...
    document
      ...
      .addEventListener('keydown', (e) => {
        ...
        if (e.key === 'Enter') {
          ...
          if (
            document.getElementById('axp_renamelayer_button_confirm').dataset
              .enabled === 'true'
          ) {
            confirmRename();
          } else {
  ...
  setAlpha(alpha, index = null) {
    if (index) {
      ...
      // 現在選択中のレイヤー
      this.layerObj[this.getLayerIndex(this.currentLayer.dataset.id)].alpha =
        alpha;
      // 不透明度に応じて目のアイコンの背景を変化させる
  ...
  getIndex() {
    ...
    if (this.currentLayer === null) {
      ...
      index =
        this.layerObj[this.getLayerIndex(this.currentLayer.dataset.id)].index;
    }
    return index;
  }
  getImage() {
          ...
          if (e.buttons === 1) {
            if (!this.axpObj.isDrawing) {
              if (e.target.classList.contains('axpc_icon_eyeON')) {
                ...
                this.layerObj[this.getLayerIndex(e.target.dataset.id)].checked =
                  false;
              } else {
                console.log('on');
              ...
              msgtext = e.target.classList.contains('axpc_icon_eyeON')
                ? '表示'
                : '非表示';
  ...
  openRenameLayerWindow() {
    ...
    // 決定ボタン無効化
    document.getElementById('axp_renamelayer_button_confirm').dataset.enabled =
      'false';
    // テキストエリアにフォーカス
    textbox.focus();
    textbox.select();
            ...
            while (skipIdx >= 0) {
              if (this.layerObj[skipIdx].mode === 'source-atop') {
                ...
                if (this.layerObj[skipIdx].checked) {
                  // safari
                  if (this.axpObj.ENV.multiCanvas) {
                    this.CANVAS.layer_ctx[skipIdx].putImageData(
                    ...
                    this.CANVAS.clip_ctx.drawImage(
                      this.CANVAS.layer_ctx[skipIdx].canvas,
                      0,
                      0,
                    );
                  } else {
...
const util = {
  ...
  swap(target) {
    ...
    for (let thatIdx = 0, len = siblings.length; thatIdx < len; thatIdx++) {
      ...
      const hitX =
        thatRectX <= targetRectX + targetW && thatRectX + thatW >= targetRectX;
      const hitY =
        targetRectY <= thatRectYHalf && targetRectY + targetH >= thatRectYHalf;
